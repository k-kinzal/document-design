import{n as e}from"./rolldown-runtime-DkW27tQK.js";import{l as t,o as n,s as r}from"./iframe-fR7HCL7S.js";var i,a,o,s,c,l,u,d,f;function p(){return(p=e((()=>{r(),i={title:`Components/Plate`,parameters:{docs:{description:{component:"A figure as a unit of explanation rather than a box with a picture in it.\n\nThe drawing is one of several parts. A reader who cannot find the conditions the figure was measured under, or cannot tell which mark the paragraph means by “this one”, has been given a picture and not an explanation. So a plate carries the drawing, its labels, a caption, a number, where the data came from, and a name the running text can point at.\n\n**Three widths.** Text is capped at the measure because a long line is hard to return from. A drawing is looked at rather than swept, and a table's columns get narrower for nothing, so a plate may leave it: `--dd-measure` → `.plate-wide` → `.plate-full`.\n\n**The number is CSS, the reference is yours.** A counter can be read where it is declared and nowhere else, so the sentence saying “see Figure 2” cannot ask the caption what it ended up being. When a generator writes one number it must write both — `.plate-unnumbered` turns the counter off so the two cannot drift apart."}}}},a=(e=520,t=`en`)=>`
  <svg class="draw" style="--dd-draw-width:${e}px" viewBox="0 0 ${e} 120" role="img"
       aria-label="${t===`ja`?`0から100の尺。68.97が変更前、96.67が上限。`:`A scale from 0 to 100. Before: 68.97. Ceiling: 96.67.`}">
    <text x="16" y="18" text-anchor="start" class="draw-cap">BEFORE / AFTER</text>
    <line class="draw-line" x1="16" y1="62" x2="${e-16}" y2="62"/>
    <circle class="plot-before" cx="${Math.round((e-32)*.69)+16}" cy="62" r="5"/>
    <circle class="plot-point" cx="${Math.round((e-32)*.967)+16}" cy="62" r="6"/>
    <text x="${Math.round((e-32)*.69)+16}" y="44" text-anchor="middle" class="draw-value draw-note">68.97</text>
    <text x="${Math.round((e-32)*.967)+16}" y="44" text-anchor="end" class="draw-value draw-accent">96.67</text>
    <text x="16" y="92" text-anchor="start" class="draw-note">0</text>
    <text x="${e-16}" y="92" text-anchor="end" class="draw-warn">${t===`ja`?`100 にはしない`:`Do not force 100`}</text>
  </svg>`,o={render:()=>n`
...
<div>
  ${t(`The roles — all one size (--dd-text-md, 14px), distinguished by weight, colour and family`,`<div class="draw-wrap"><svg class="draw" style="--dd-draw-width:660px" viewBox="0 0 660 260" role="img"
       aria-label="Examples of the text roles in draw.css.">
      <text x="20" y="24" text-anchor="start" class="draw-cap">DRAW ROLES</text>
      ${[[`.draw-label`,`a name on a box`,`draw-label`],[`.draw-strong`,`the one it is about`,`draw-strong`],[`.draw-note`,`a second line`,`draw-note`],[`.draw-value`,`29 / 30`,`draw-value`],[`.draw-mono`,`BISON-RUNTIME-001`,`draw-mono draw-note`],[`.draw-accent`,`After`,`draw-accent`],[`.draw-warn`,`Left empty`,`draw-warn`]].map(([e,t,n],r)=>`<text x="20" y="${62+r*28}" text-anchor="start" class="draw-mono draw-note">${e}</text>
             <text x="300" y="${62+r*28}" text-anchor="start" class="${n}">${t}</text>`).join(``)}
    </svg></div>`)}
  ${t(`The marks`,`<div class="draw-wrap"><svg class="draw" style="--dd-draw-width:660px" viewBox="0 0 660 96" role="img"
    ...
    <div class="compare compare-draw">
      <div class="was"><span class="cap">Before · 9 empty</span>
        <div class="draw-wrap"><svg class="draw" style="--dd-draw-width:268px" viewBox="0 0 268 168" role="img" aria-label="All nine units are empty.">
          ...
          ${[0,1,2,3,4].map(e=>`<rect class="draw-box-open tone-neutral" x="${36+e*36}" y="36" width="28" height="28" rx="6"/>`).join(``)}
          ${[0,1,2,3].map(e=>`<rect class="draw-box-open tone-neutral" x="${54+e*36}" y="72" width="28" height="28" rx="6"/>`).join(``)}
      ...
      <div class="now"><span class="cap">After · 7 linked / 1 tested / 1 left</span>
        <div class="draw-wrap"><svg class="draw" style="--dd-draw-width:340px" viewBox="0 0 340 168" role="img" aria-label="Seven linked, one new scenario, and one left empty.">
          ...
          ${[0,1,2,3,4,5,6].map(e=>`<rect class="draw-box-toned tone-accent" x="${36+e*32}" y="36" width="24" height="24" rx="6"/>`).join(``)}
...
<div class="prose" lang="ja">
  ...
  <figure class="plate plate-wide" id="plate-anatomy-ja">
    <div class="draw-wrap">${a(520,`ja`)}</div>
...
</div>`},f=[`Anatomy`,`Widths`,`Table`,`Labels`,`Panels`,`JapaneseCaption`],o.parameters={...o.parameters,docs:{...o.parameters?.docs,source:{originalSource:`{
  render: () => html\`
<div class="prose" lang="en">
  <p>Nine of 29 source units were empty. <a class="ref" href="#plate-anatomy">Figure 1</a> shows the result:
  96.67 is the honest ceiling. Do not reach 100 by relabelling gaps as unsupported.</p>
...
}`,...o.parameters?.docs?.source},description:{story:`The parts, and a sentence that points at them.`,...o.parameters?.docs?.description}}},s.parameters={...s.parameters,docs:{...s.parameters?.docs,source:{originalSource:`{
  render: () => html\`
<div class="prose">
  <p>Running text stops at <code>--dd-measure</code>: 36 characters of Japanese, 77 of latin.
  A plate may stay there, or leave.</p>
...
}`,...s.parameters?.docs?.source},description:{story:`The measure, the figure width, and the column.`,...s.parameters?.docs?.description}}},c.parameters={...c.parameters,docs:{...c.parameters?.docs,source:{originalSource:`{
  render: () => html\`
<div class="prose">
  <figure class="plate plate-table plate-wide">
    <figcaption>What each severity means, and how many carry it.</figcaption>
...
}`,...c.parameters?.docs?.source},description:{story:`A table's caption goes above it.`,...c.parameters?.docs?.description}}},l.parameters={...l.parameters,docs:{...l.parameters?.docs,source:{originalSource:`{
  render: () => html\`
<div>
  \${specimen("The roles — all one size (--dd-text-md, 14px), distinguished by weight, colour and family", \`<div class="draw-wrap"><svg class="draw" style="--dd-draw-width:660px" viewBox="0 0 660 260" role="img"
       aria-label="Examples of the text roles in draw.css.">
      <text x="20" y="24" text-anchor="start" class="draw-cap">DRAW ROLES</text>
      \${[[".draw-label", "a name on a box", "draw-label"], [".draw-strong", "the one it is about", "draw-strong"], [".draw-note", "a second line", "draw-note"], [".draw-value", "29 / 30", "draw-value"], [".draw-mono", "BISON-RUNTIME-001", "draw-mono draw-note"], [".draw-accent", "After", "draw-accent"], [".draw-warn", "Left empty", "draw-warn"]].map(([name, sample, cls], i) => \`<text x="20" y="\${62 + i * 28}" text-anchor="start" class="draw-mono draw-note">\${name}</text>
             <text x="300" y="\${62 + i * 28}" text-anchor="start" class="\${cls}">\${sample}</text>\`).join("")}
    </svg></div>\`)}
  \${specimen("The marks", \`<div class="draw-wrap"><svg class="draw" style="--dd-draw-width:660px" viewBox="0 0 660 96" role="img"
       aria-label="Examples of the drawing marks in draw.css.">
      <rect class="draw-box" x="16" y="28" width="120" height="40" rx="8"/>
      <text x="76" y="54" text-anchor="middle" class="draw-label">.draw-box</text>
      <rect class="draw-box-toned" style="--dd-tone:var(--dd-accent);--dd-tone-tint:var(--dd-accent-tint)" x="164" y="28" width="150" height="40" rx="8"/>
...
}`,...l.parameters?.docs?.source},description:{story:`Every role a label can take, at the size the system gives it.`,...l.parameters?.docs?.description}}},u.parameters={...u.parameters,docs:{...u.parameters?.docs,source:{originalSource:`{
  render: () => html\`
<div class="prose" lang="en">
  <p>Before and after form one explanation, with one number and one caption. Keep the two drawings separate:
  putting both in a single fixed-width drawing makes readers in narrow containers remember one side while scrolling to the other.</p>
...
}`,...u.parameters?.docs?.source},description:{story:`Two drawings, one figure: a comparison that never has to be scrolled.`,...u.parameters?.docs?.description}}},d.parameters={...d.parameters,docs:{...d.parameters?.docs,source:{originalSource:`{
  render: () => html\`
<div class="prose" lang="ja">
  <p>出典 29 単位のうち 9 が空だった。到達点は <a class="ref" href="#plate-anatomy-ja">図 1</a> のとおりで、
  96.67 が正直な上限であり、100 は非対応への付け替えで作らない。</p>
...
}`,...d.parameters?.docs?.source},description:{story:`Japanese figure prefix, reference, and caption at the same drawing scale.`,...d.parameters?.docs?.description}}}})))()}p();export{o as Anatomy,d as JapaneseCaption,l as Labels,u as Panels,c as Table,s as Widths,f as __namedExportsOrder,i as default};