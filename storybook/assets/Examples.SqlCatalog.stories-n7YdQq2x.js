import{n as e}from"./rolldown-runtime-DkW27tQK.js";import{o as t,s as n}from"./iframe-fR7HCL7S.js";var r,i,a,o,s,c,l,u,d,f,p,m;function h(){return(h=e((()=>{n(),r={title:`Examples/SQL catalog`,parameters:{docs:{description:{component:"ztd-query-php's catalog overview for WordPress, rebuilt on this stylesheet. Same figures: 844 statements, 12 tables, 240 functions, 77 files, 978 findings. The `.k-select` family is gone — kinds are tone classes now, and the coverage bar is `.meter` rather than a second `.stack` that meant something else in the other project."}}}},i=[[`SELECT`,`blue`,639],[`ALTER`,`indigo`,63],[`UPDATE`,`violet`,44],[`INSERT`,`teal`,35],[`DELETE`,`pink`,28],[`UNKNOWN`,`slate`,20],[`DROP`,`indigo`,9],[`OTHER`,`slate`,3],[`CREATE`,`indigo`,2],[`SHOW`,`slate`,1]],a=[[`{$}cache_data`,`13 · 4 read · 8 write`],[`{$}items`,`7 · 4 read · 2 write`],[`{$}posts`,`3 · 3 read · 0 write`],[`information_schema.TABLES`,`2 · 2 read · 0 write`],[`{$}linkcategories`,`2 · 1 read · 0 write`],[`{$}categories`,`1 · 0 read · 0 write`]],o=[[`WP_User_Search`,`36 statements`],[`wpdb`,`28 statements`],[`WP_Query`,`23 statements`],[`SimplePie\\Cache\\MySQL`,`21 statements`],[`WP_Term_Query`,`20 statements`],[`WP_Comment_Query`,`18 statements`]],s=[[`wp-admin/includes/upgrade.php`,`162 statements`],[`wp-admin/includes/deprecated.php`,`57 statements`],[`wp-includes/post.php`,`49 statements`],[`wp-includes/general-template.php`,`39 statements`],[`wp-includes/taxonomy.php`,`39 statements`],[`wp-includes/meta.php`,`31 statements`]],c=[[`dynamic-sql`,`warn`,`medium`,709,`A value is spliced into the statement text instead of being bound.`],[`analysis-incomplete`,`neutral`,`low`,239,`A cycle or an analysis budget stopped the search before it closed.`],[`unresolved-sql`,`neutral`,`low`,25,`The statement text could not be reconstructed at all.`],[`call-not-analyzed`,`neutral`,`low`,5,`A call that carries a statement was found but never examined.`]],l=[[`WP_User_Search::query`,`wp-admin/includes/deprecated.php · 36 medium`],[`wp_get_archives`,`wp-includes/general-template.php · 34 medium`],[`pre_schema_upgrade`,`wp-admin/includes/upgrade.php · 25 medium`],[`drop_index`,`wp-admin/includes/upgrade.php · 21 medium`],[`post_exists`,`wp-admin/includes/post.php · 20 medium`],[`export_wp`,`wp-admin/includes/export.php · 19 medium`]],u=[[`resolved`,`tone-ok`,35,`The statement text is fully determined.`,`tone-ok`],[`external-input`,`tone-danger`,0,`The values were followed to runtime input, so the text cannot be fixed.`,`tone-danger`],[`incomplete-model`,`chip-ghost`,709,`A dependency the analyzer does not model was reached.`,`is-open`],[`incomplete`,`chip-ghost`,95,`A cycle or an analysis budget stopped the search before it closed.`,`is-open|--dd-hatch:45deg`],[`not-analyzed`,`tone-neutral`,5,`The call was found but never examined, so nothing was read from it.`,`tone-neutral`]],d=e=>e.map(([e,t])=>`<li><a href="#">${e}</a><span class="peek-figures">${t}</span></li>`).join(``),f={render:()=>t`
    <div class="doc">
      <nav class="sidebar">
        ...
        <main class="content">
          ...
          <div class="cards">
            ...
            <section class="card">
              ...
              <ol class="peek peek-chips">
                ${i.map(([e,t,n])=>`<li><a class="chip tone-${t}" href="#">${e}</a><span class="peek-figures">${n}</span></li>`).join(``)}
              </ol>
          ...
          <div class="split">
            ...
            <section class="aside">
              ...
              <ol class="peek">
                ${l.map(([e,t])=>`<li><a class="mono" href="#">${e}</a><span class="peek-figures">${t}</span></li>`).join(``)}
              </ol>
          ...
          <ul class="legend">
            ${u.map(([e,t,n,r,i])=>`
              <li>
                <span class="meter-legend-swatch ${i.split(`|`)[0]}"${i.includes(`|`)?` style="${i.split(`|`)[1]}"`:``}></span>
                <a class="chip ${t}" href="#" title="${r}">${e}</a>
                <span class="meter-legend-count">${n}</span>
                <span class="meter-legend-description">${r}</span>
              </li>`).join(``)}
          </ul>
        </main>
      </div>
        ...
        <main class="content">
          ...
          <div class="facets" data-dd-facets="#stmts">
            ...
            <div class="facet-group">
              ${i.slice(0,5).map(([e,t,n])=>`<button class="chip facet tone-${t}" data-dd-facet="kind:${e.toLowerCase()}">${e}<span class="facet-count">${n}</span></button>`).join(``)}
            </div>
            <div class="facet-group">
          ...
          <ul class="rows" id="stmts">
            ${[[`select`,`resolved`,`blue`,`SELECT`,`SELECT * FROM {$}posts WHERE post_status = 'publish' AND post_type = %s ORDER BY post_date DESC LIMIT 10`,`wp-includes/post.php:1240`,`get_posts`],[`select`,`open`,`blue`,`SELECT`,`SELECT option_value FROM {$}options WHERE option_name = %s LIMIT 1`,`wp-includes/option.php:168`,`get_option`],[`update`,`open`,`violet`,`UPDATE`,`UPDATE {$}options SET option_value = %s WHERE option_name = %s`,`wp-includes/option.php:412`,`update_option`],[`insert`,`resolved`,`teal`,`INSERT`,`INSERT INTO {$}postmeta (post_id, meta_key, meta_value) VALUES (%d, %s, %s)`,`wp-includes/meta.php:96`,`add_metadata`],[`alter`,`open`,`indigo`,`ALTER`,`ALTER TABLE {$}posts ADD INDEX post_name (post_name(191))`,`wp-admin/includes/upgrade.php:2104`,`pre_schema_upgrade`],[`delete`,`open`,`pink`,`DELETE`,`DELETE FROM {$}postmeta WHERE post_id = %d AND meta_key = %s`,`wp-includes/meta.php:412`,`delete_metadata`]].map(([e,t,n,r,i,a,o])=>`
              <li class="row" data-dd-kind="${e}" data-dd-resolution="${t}">
                ...
                <div class="row-meta">
                  <a href="#">${a}</a>
                  <a href="#">${o}</a>
                  ${t===`open`?`<span class="chip chip-sm tone-warn">dynamic-sql</span>`:``}
                </div>
              </li>`).join(``)}
          </ul>
        </main>
      </div>
    </div>`},m=[`Overview`,`Statements`],f.parameters={...f.parameters,docs:{...f.parameters?.docs,source:{originalSource:`{
  render: () => html\`
    <div class="doc">
      <nav class="sidebar">
        <div class="sidebar-section">
      ...
      <div class="main">
        ...
        <main class="content">
          ...
          <div class="cards">
            ...
            <section class="card">
              ...
              <p class="card-description">Every statement, to narrow down by what it does, how far the analysis got
                and what was reported.</p>
              <ol class="peek peek-chips">
                \${KINDS.map(([label, tone, n]) => \`<li><a class="chip tone-\${tone}" href="#">\${label}</a><span class="peek-figures">\${n}</span></li>\`).join("")}
              </ol>
              <p class="card-more"><a href="#">All 844 statements</a></p>
            </section>
          </div>
          ...
          <div class="split">
            <div class="table-wrap">
              <table>
                <thead><tr><th>Rule</th><th class="tight">Severity</th><th class="num">Statements</th><th>What it reports</th></tr></thead>
                <tbody>
                  \${RULES.map(([rule, tone, sev, n, what]) => \`
                    <tr>
            ...
            <section class="aside">
              <h3>Functions issuing flagged statements</h3>
              <ol class="peek">
                \${HOTSPOTS.map(([fn, where]) => \`<li><a class="mono" href="#">\${fn}</a><span class="peek-figures">\${where}</span></li>\`).join("")}
              </ol>
              <p class="card-more"><a href="#">Every flagged function</a></p>
            </section>
          </div>
          ...
          <ul class="legend">
            \${RESOLUTIONS.map(([name, cls, n, note, mark]) => \`
              <li>
      ...
      <div class="main">
        ...
        <main class="content">
          ...
          <div class="facets" data-dd-facets="#stmts">
            <input type="search" class="input" placeholder="Filter…" data-dd-facet-search>
            <div class="facet-group">
              \${KINDS.slice(0, 5).map(([label, tone, n]) => \`<button class="chip facet tone-\${tone}" data-dd-facet="kind:\${label.toLowerCase()}">\${label}<span class="facet-count">\${n}</span></button>\`).join("")}
            </div>
            <div class="facet-group">
              <button class="chip facet tone-ok" data-dd-facet="resolution:resolved">resolved<span class="facet-count">35</span></button>
              <button class="chip facet tone-neutral" data-dd-facet="resolution:open">lower bound<span class="facet-count">812</span></button>
          ...
          <ul class="rows" id="stmts">
            \${[["select", "resolved", "blue", "SELECT", "SELECT * FROM {$}posts WHERE post_status = 'publish' AND post_type = %s ORDER BY post_date DESC LIMIT 10", "wp-includes/post.php:1240", "get_posts"], ["select", "open", "blue", "SELECT", "SELECT option_value FROM {$}options WHERE option_name = %s LIMIT 1", "wp-includes/option.php:168", "get_option"], ["update", "open", "violet", "UPDATE", "UPDATE {$}options SET option_value = %s WHERE option_name = %s", "wp-includes/option.php:412", "update_option"], ["insert", "resolved", "teal", "INSERT", "INSERT INTO {$}postmeta (post_id, meta_key, meta_value) VALUES (%d, %s, %s)", "wp-includes/meta.php:96", "add_metadata"], ["alter", "open", "indigo", "ALTER", "ALTER TABLE {$}posts ADD INDEX post_name (post_name(191))", "wp-admin/includes/upgrade.php:2104", "pre_schema_upgrade"], ["delete", "open", "pink", "DELETE", "DELETE FROM {$}postmeta WHERE post_id = %d AND meta_key = %s", "wp-includes/meta.php:412", "delete_metadata"]].map(([kind, res, tone, label, sql, site, fn]) => \`
...
}`,...p.parameters?.docs?.source},description:{story:`The listing the overview leads to, with its facets live.`,...p.parameters?.docs?.description}}}})))()}h();export{f as Overview,p as Statements,m as __namedExportsOrder,r as default};