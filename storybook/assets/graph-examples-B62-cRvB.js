import{n as e}from"./rolldown-runtime-DkW27tQK.js";var t,n,r,i,a,o,s,c,l,u,d,f,p,m,h,g,_,v,y,b,x,S,C,w,T,E,D,O,k,A;function j(){return(j=e((()=>{t=[[`MySQL::save`,8],[`MySQL::__construct`,1],[`MySQL::load`,1],[`MySQL::mtime`,1],[`MySQL::touch`,1],[`MySQL::unlink`,1]],n=[90,120,135,142,146,156,204,208,216,240,297,317,335],r=`WordPress SQL catalog snapshot · cache_data · SimplePie/Cache/MySQL.php`,i=`<figure class="plate plate-wide">
  ...
  <ul class="bars">
${t.map(([e,t])=>`    <li class="bar-row">
      <span class="bar-label mono">${e}</span>
      <span class="bar-track" aria-hidden="true"><span class="bar-fill" style="--dd-bar:${t/8*100}%"></span></span>
      <span class="bar-value">${t} ${t===1?`statement`:`statements`}</span>
    </li>`).join(`
`)}
  </ul>
  <figcaption>MySQL::save contains 8 of the 13 statements; each other caller contains one.
    <span class="plate-source">${r}. Counts describe static call sites, not execution frequency.</span>
  </figcaption>
</figure>`,a=`<figure class="plate plate-wide">
...
</figure>`,o=`<figure class="plate plate-wide">
...
</figure>`,s=e=>64+e*1.64,c=e=>244-e*14,l=n.map((e,t)=>`H${s(e).toFixed(2)}V${c(t+1)}`).join(` `),u=[0,100,200,300,400].map(e=>[e,n.filter(t=>t<=e).length]),d=`<figure class="plate plate-wide">
  ...
  <div class="draw-wrap" role="region" aria-label="Cumulative statement plot" tabindex="0">
    <svg class="draw" style="--dd-draw-width:768px" viewBox="0 0 768 320" role="img" aria-label="Cumulative statements referencing cache_data by source line: line 0, zero; line 100, one; line 200, six; line 300, eleven; line 400, thirteen. Steps occur at the 13 observed call sites.">
      ...
      <path class="plot-line" d="M64 244 ${l} H720"/>
${u.map(([e,t])=>`      <circle class="plot-point" cx="${s(e)}" cy="${c(t)}" r="4"/>
      <text class="draw-value" x="${s(e)}" y="${c(t)-14}" text-anchor="middle">${t}</text>
      <text class="draw-note" x="${s(e)}" y="270" text-anchor="middle">${e}</text>`).join(`
`)}
  ...
  <figcaption>The steps show observed call sites, not runtime activity or a time series.
    <span class="plate-source">${r}. 13 statements at lines ${n.join(`, `)}.</span>
  </figcaption>
</figure>`,f=`<figure class="plate plate-wide">
...
</figure>`,p=[[1,2],[3,4],[5,6],[7,8]].map(([e,n])=>({lo:e,hi:n,count:t.filter(([,t])=>t>=e&&t<=n).length})),m=`<figure class="plate plate-wide">
...
${p.map(({lo:e,hi:t,count:n},r)=>`      ${n?`<rect class="plot-bin" x="${64+r*160}" y="${244-n*36}" width="160" height="${n*36}"/>`:`<path class="plot-zero" d="M${64+r*160} 244h160"/>`}
      <text class="draw-value" x="${144+r*160}" y="${230-n*36}" text-anchor="middle">${n} ${n===1?`caller`:`callers`}</text>
      <text class="draw-note" x="${144+r*160}" y="272" text-anchor="middle">${e}–${t}</text>`).join(`
`)}
  ...
  <figcaption>Five callers contain one statement each; one contains eight. Empty bins are observed zeros, not missing measurements.
    <span class="plate-source">${r}. All 6 callers, 13 statements; bin width: 2 statements. This small sample describes one table only.</span>
  </figcaption>
</figure>`,h=`<figure class="plate plate-wide">
...
</figure>`,g=`<svg class="draw" style="--dd-draw-width:240px" viewBox="0 0 240 168" role="img" aria-label="Source coverage before 68.97 percent, after 96.67 percent. Full conditions in the comparison example.">
...
</svg>`,_=n.map((e,t)=>`H${(32+e*.46).toFixed(2)}V${(128-(t+1)*96/13).toFixed(2)}`).join(` `),v=`<svg class="draw" style="--dd-draw-width:240px" viewBox="0 0 240 168" role="img" aria-label="13 cumulative statements across source lines 0 to 400. Full observations in the line plot example.">
  <text class="draw-note" x="8" y="18">Statements by source line</text>
  <path class="plot-axis" d="M32 28V128H216"/>
  <path class="plot-line" d="M32 128 ${_} H216"/>
...
</svg>`,y=`<svg class="draw" style="--dd-draw-width:240px" viewBox="0 0 240 176" role="img" aria-label="Callers in two-statement bins: 1–2, five; 3–4, zero; 5–6, zero; 7–8, one.">
  <text class="draw-note" x="8" y="18">Callers · n = 6</text>
  <path class="plot-axis" d="M16 32V120H224"/>
${p.map(({lo:e,hi:t,count:n},r)=>`  ${n?`<rect class="plot-bin" x="${16+r*52}" y="${120-n*16}" width="52" height="${n*16}"/>`:`<path class="plot-zero" d="M${16+r*52} 120h52"/>`}
  <text class="draw-value" x="${42+r*52}" y="${112-n*16}" text-anchor="middle">${n}</text>
  <text class="draw-note" x="${42+r*52}" y="143" text-anchor="middle">${e}–${t}</text>`).join(`
`)}
  <text class="draw-note" x="120" y="168" text-anchor="middle">Statements per caller</text>
</svg>`,b=`<svg class="draw" style="--dd-draw-width:240px" viewBox="0 0 240 168" role="img" aria-label="Reader’s task: find one entry, choose Catalog; understand the whole, choose Report.">
...
</svg>`,x=[{x:16,w:148,value:`844`,note:`statements`},{x:212,w:148,value:`callers`,note:`resolved`},{x:408,w:148,value:`dependencies`,note:`followed`}],S=[{x:18,w:144,cx:90,value:`5`,note:`not analyzed`,tone:` tone-danger`},{x:214,w:144,cx:286,value:`709`,note:`no model`,tone:``},{x:410,w:144,cx:482,value:`95`,note:`cycle or limit`,tone:``}],C=`<figure class="plate plate-wide" id="fig-stops">
...
${x.map(e=>`      <rect class="draw-box" x="${e.x}" y="24" width="${e.w}" height="56" rx="8"/>
      <text class="draw-value" x="${e.x+e.w/2}" y="46" text-anchor="middle">${e.value}</text>
      <text class="draw-note" x="${e.x+e.w/2}" y="66" text-anchor="middle">${e.note}</text>`).join(`
`)}
...
${S.map((e,t)=>`      <path class="draw-line draw-arrow" d="M${e.cx} 84V144"/>
      ...
      <g class="mark mark-open${e.tone}" transform="translate(${e.x+e.w+20} 174)"><circle/><text>${t+1}</text></g>`).join(`
`)}
    </svg>
  </div>
...
</figure>`,w=`<svg class="draw" style="--dd-draw-width:240px" viewBox="0 0 240 152" role="img" aria-label="Of 844 statements, 35 are fully determined; 809 stop at one of three exits.">
...
</svg>`,T=e=>16+e*108,E=e=>T(e)+46,D=[[`draw-box`,`thing`],[`draw-box-toned tone-accent`,`identity`],[`draw-box-alt tone-accent`,`other state`],[`draw-box-open`,`left open`],[`draw-box-open tone-neutral`,`a hole`],[`draw-fill tone-accent`,`a region`],[`draw-group`,`grouped`]],O=[[`draw-line draw-arrow`,`directed`],[`draw-line-open draw-arrow`,`not established`],[`draw-guide`,`apparatus`],[`draw-line draw-focus tone-accent`,`the subject`],[`leader`,`names a place`]],k=`<figure class="plate plate-wide" id="fig-marks">
...
${D.map(([e,t],n)=>`      <rect class="${e}" x="${T(n)}" y="28" width="92" height="44" rx="6"/>
      <text class="draw-note" x="${E(n)}" y="90" text-anchor="middle">${t}</text>`).join(`
`)}
      <text class="draw-cap" x="16" y="122">LINES</text>
${O.map(([e,t],n)=>`      <path class="${e}" d="M${T(n)} 142H${T(n)+92}"/>${e===`leader`?`\n      <circle class="leader-foot" cx="${T(n)}" cy="142"/>`:``}
      <text class="draw-note" x="${E(n)}" y="172" text-anchor="middle">${t}</text>`).join(`
`)}
    </svg>
  </div>
...
</figure>`,A=`<svg class="draw" style="--dd-draw-width:240px" viewBox="0 0 240 120" role="img" aria-label="A thing, a thing with identity and something left open, joined by a directed line and a dashed one.">
...
</svg>`})))()}export{w as a,m as c,k as d,A as f,v as g,d as h,C as i,y as l,f as m,h as n,o,a as p,b as r,g as s,i as t,j as u};