...
| Paper size and page numbers | `data-dd-paper="a4"` / `"letter"` on the root, mapped to named `@page` rules with margin-box page numbers | The stylesheet does not know what is in the printer; a paper meant to be a PDF does. The generated paper this was measured on had to add its own `@page`, fixed-height page boxes and a hand-positioned footer. |
| Printed URLs | `data-dd-print-urls="sources"` / `"none"`; `.sources` spells out each address on its own line on paper | The default writes every external link out in place, which is right for a README and wrong for sixty citations: a line per source in the running text, then the same address again in the list. |
| Citations | `.cite` in the text, `.sources` at the end, numbered by the list-item counter | A figure is looked at, so `.ref` carries an underline. A source is consulted later, so the pointer stays out of the line: a bracketed number at count size, no underline. `start` and `value` still work. |
| Chronologies | `.sheet .timeline` takes the report's reading size | A timeline at 13px under notes set at 16px was the one block that dropped below the reading size — for a career told a few sentences at a time, in the genre that exists to be read. |
| A lead made of words | `.hero > .figures` | A profile, a project or a theme has no before and after. Three figures in the hero's slot keep the opening one group, closed by the hero's rule. |
...
- **A block gap the layout layer cannot win belongs in the component layer, after every component it spaces.** Components declare their margins with shorthands (`.flow { margin: 0 }`, `.definitions { margin: 0 0 … }`) in `dd.component`, so a `margin-top` written in `dd.layout` loses whatever its selector says. The report field's inter-block baseline (`.sheet .field > :is(.note, .lead, …) + :is(.compare, .flow, …)`) therefore sits in `components/composition.css`, which is imported after every component it spaces. Measured on a generated report: a `.note` followed by a `.compare` had no gap at all. Headings are not in the preceding set: written as `* + …`, the rule doubled the product page's title-to-prose gap from 14 to 28px.
...
- **Do not lay a list item out as a grid if the list numbers it.** `.sources > li { display: grid }` silently stopped the `list-item` counter: every entry read "[0]", because a box that is not a list item is not counted as one. Keep `display: list-item` and position the number into a gutter.
- **Do not draw anything that must survive pagination with a positioned box.** The timeline's spine was an absolutely positioned pseudo-element the height of the list; a positioned box is not fragmented, so on paper it appeared on the first page the list started on and on no page after. On paper the spine is a border on the list, which every fragment draws.
- **Verify pagination by paginating.** Print media is not pagination: `break-after: avoid-page` does nothing until a page ends. `packages/doc-site/tests/pdf.spec.mjs` prints the long example with Chromium and reads the PDF back — page size, page numbers, no heading alone at the foot of a page, every entry and source present, the last paragraph on the last page. The report's `.label` and `.lead` are a `<div>` and a `<p>`; the heading break rules did not reach them until they were named.
- **A citation is not a figure reference.** `.ref` underlines because a figure is looked at; a sentence with three `.ref`s would be three links. `.cite` is a bracketed number at count size in the muted ink with no underline, and hover shows it is a link. One document can use both; they point at different things.
- **Sources keep their own language.** In the English paper the cited titles stay Japanese and carry `lang="ja"`, because a title is quoted, not translated. The content test allows Japanese on English pages only inside those links.
