...
- **A block gap the layout layer cannot win belongs in the component layer, after every component it spaces.** Components declare their margins with shorthands (`.flow { margin: 0 }`, `.definitions { margin: 0 0 … }`) in `dd.component`, so a `margin-top` written in `dd.layout` loses whatever its selector says. The report field's inter-block baseline (`.sheet .field > * + :is(.compare, .flow, …)`) therefore sits in `components/composition.css`, which is imported after every component it spaces. Measured on a generated report: a `.note` followed by a `.compare` had no gap at all.
