...
export function document({ title, description, path = '', body, search = false, noindex = false, development = false }) {
  const depth = path.split('/').filter(Boolean).length;
  const base = depth ? '../'.repeat(depth) : './';
  const url = new URL(path, site.url).href;
  const pageTitle = path || noindex ? `${title} — doc-ui` : site.title;
  const ld = [{ '@context': 'https://schema.org', '@type': path ? 'TechArticle' : 'WebPage', '@id': url, url, name: pageTitle, headline: title, description, inLanguage: 'en', author: { '@type': 'Person', name: 'k-kinzal', url: 'https://github.com/k-kinzal' }, ...(path ? {} : { mainEntity: { '@type': 'SoftwareSourceCode', name: 'doc-ui', description: site.description, codeRepository: site.repository, programmingLanguage: ['CSS', 'HTML', 'JavaScript'], license: 'https://opensource.org/license/mit' } }) }];
  if (path && !noindex) ld.push({ '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'doc-ui', item: site.url }, ...(path.startsWith('components/') && path !== 'components/' ? [{ '@type': 'ListItem', position: 2, name: 'Components', item: new URL('components/', site.url).href }] : []), { '@type': 'ListItem', position: path.startsWith('components/') && path !== 'components/' ? 3 : 2, name: title, item: url }] });
  return `<!doctype html>
...
${path ? '' : `<meta property="og:image" content="${site.url}assets/og.png">
...
<meta name="twitter:card" content="${path ? 'summary' : 'summary_large_image'}">
<meta name="twitter:title" content="${escape(pageTitle)}">
<meta name="twitter:description" content="${escape(description)}">
${path ? '' : `<meta name="twitter:image" content="${site.url}assets/og.png">
...
${search ? `<script vite-ignore src="${base}assets/search.js" defer></script>` : ''}
