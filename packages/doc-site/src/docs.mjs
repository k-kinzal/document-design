...
import { site, escape, code, stylesheet, cssURL, jsURL, languageLink } from './site.mjs';
...
const stories = new Map(Object.entries({
  ...
  "examples-paper": "examples-paper--paper"
}));
...
function shell({ base, title, active, content, lang = 'en' }) {
...
function componentPage(c, index, lang) {
  ...
  return { path: `components/${c.slug}/`, title: `${c.name} — ${c.label}`, description: `${escape(c.description)} Live examples, copyable HTML, classes, and usage guidance for the doc-ui ${c.name} component.`, search: true, body: shell({ lang, base: '../../', title: c.name, active: c.slug, content }) };
}
...
<section id="language"><h2>Set the document language</h2><div class="prose"><p>Declare <code>lang="en"</code> for English or <code>lang="ja"</code> for Japanese on the HTML element. The language selects Japanese phrase-aware heading breaks and figure captions. Body text keeps its natural spacing; headings and short labels use proportional metrics.</p><p>Use the language link in the header to compare the same page in English and Japanese. Try <a href="../components/prose/">Prose</a> for reading text and <a href="../components/report/">Report</a> for the larger type scale.</p></div></section>
...
<section id="print"><h2>Print a long paper to PDF</h2><div class="prose"><p>A paper that will be read as a PDF names its paper size on the root element. The stylesheet then sets the sheet size and margins and numbers every page, and no print CSS of your own is needed.</p></div>${code('<html lang="ja" data-dd-paper="a4" data-dd-print-urls="sources">', 'print-attributes')}<div class="prose"><ul><li><code>data-dd-paper</code> takes <code>a4</code> or <code>letter</code>. Leave it out to print at the size the reader chooses.</li><li><code>data-dd-print-urls</code> chooses where link addresses are printed: after each link in the text by default, only in the <a href="../components/sources/">sources list</a> with <code>sources</code>, or nowhere with <code>none</code>.</li><li>Headings, section labels, timeline entries, and sources stay with what follows them. Tables and code blocks break where they must.</li><li>Print from the browser with “Save as PDF”, or generate the file with a headless browser. Keep background graphics off; the stylesheet redraws chips and bars as outlines.</li></ul><p>The complete example below is a profile paper with a chronology across several pages, sixty-two sources, and long Japanese titles. It uses only the stylesheet above.</p></div><div class="actions"><a class="btn" href="../examples/paper.html">Open the paper ↗</a><a href="../examples/paper.html" download="paper.html">Download the HTML ↓</a></div></section>
<section><h2>Browser support & versions</h2><p>Designed for current Chrome, Edge, Firefox, and Safari. The CSS uses <code>light-dark()</code>, <code>color-mix()</code>, cascade layers, container queries, and subgrid.</p><p>Full versions such as <code>/v1.0.0/</code> and 40-character commit paths are immutable. <code>/v1.0/</code> and <code>/v1/</code> follow compatible releases; <code>/latest/</code> follows main. This site follows the latest stable release. Breaking changes belong in a new major version.</p><p>This stylesheet owns its document. When embedding it into an existing application, use a separate document or check the effect of its base styles on your surrounding UI.</p></section>
...
export function docs(lang = 'en') { return [startPage(lang), indexPage(lang), ...components.map((c, i) => componentPage(c, i, lang))]; }
