/* Fixed observations, not generated demo numbers. Keep these standalone HTML
 * specimens local to this package; doc-site owns its own documentation copy.
 * Provenance: graph-examples.md. No chart runtime is required by consumers. */
...
/*
 * Where a statement stops.
 *
 * The callout figure, on the resolution counts the WordPress catalog actually
 * reports: 35 + 709 + 95 + 5 = 844. The figure is drawn around the 809 that do
 * not arrive, because that is what the reader has to know before reading any
 * other number on the page — and because a figure showing only the 35 would be
 * the invented-96%-coverage failure again, in a picture this time.
 *
 * Numbers in the drawing, names in the key. A drawing has room for "709" and
 * not for "the resolver stops where a call leaves the analysed set", and the
 * second one is the half that is a sentence: it reflows, it prints, it can be
 * searched, and it is not baked into a fixed-width picture of words.
 */
export const drawDefsMarkup = `<svg class="draw-defs" aria-hidden="true" focusable="false">
...
/* The same figure at index size. The marks stay 22px, because a callout that
 * shrinks with its drawing is a callout the reader cannot read — which is the
 * argument components/draw.css makes for labels, and a number in a disc is a
 * label with nowhere to reflow to. */
