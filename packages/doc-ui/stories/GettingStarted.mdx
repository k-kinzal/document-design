...
English is the project's default language, including these docs and the default
stories. Japanese output has dedicated typography and localized controls. Stories
named **Japanese** demonstrate those features with `lang="ja"`; use `lang="en"`
...
To put your CSS **between** two of the library's layers, declare a sublayer of
`dd` and restate the whole order first:
...
A top-level layer declared after `dd` outranks everything inside `dd` — it can
